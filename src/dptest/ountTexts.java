...
/**
 * 为了 打出 �?个字母，Alice �?�? 按 对应字�? i 次，i 是该字母在这个按键上�?处的位置�?
 * 比方说，为了按出字母 's' ，Alice �?要按 '7' 四次。类似的�? Alice �?要按 '5' 两次得到字母  'k' �?
 * 注意，数字�?'0' 和�?'1' 不映射到任何字母，所以 Alice 不 使用它们�??
 * 但是，由于传输的错误，Bob 没有收到 Alice 打字的字母信息，反�?�收到了 按键的字符串信息 �?
 * 比方说，Alice 发出的信息为 "bob" ，Bob 将收到字符串 "2266622" �?
 * 给你�?个字符串 pressedKeys ，表�? Bob 收到的字符串，请你返�? Alice 总共可能发出多少种文字信息 �??
...
public class ountTexts {
    ...
    //f [n] 为同�?个字母的连续长度 n 含有的组合可能数
    //对于3个字母的数字�? f[n] = f[n-1] + f[n-2] + f[n-3]
    //对于4个字母的数字7�?9,�? f[n] = f[n-1] + f[n-2] + f[n-3] + f[n-4]
    static {
        three[0] = 1;
        three[1] = 1;
        three[2] = 2;
