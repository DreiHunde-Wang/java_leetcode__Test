...
/**
 * 给你�?个数组 nums，我们可以将它按�?个非负整�? k 进行轮调�?
 * 这样可以使数组变为 [nums[k], nums[k + 1], ... nums[nums.length - 1], nums[0], nums[1], ..., nums[k-1]] 的形式�?�此后，任何值小于或等于其索引的项都可以记作�?分�??
 * 例如，数组为 nums = [2,4,1,3,0]，我们按 k = 2 进行轮调后，它将变成 [1,3,0,2,4]�?
 * 这将记为 3 分，因为 1 > 0 [不计分]�?3 > 1 [不计分]�?0 <= 2 [�? 1 分]�?
 * 2 <= 3 [�? 1 分]�?4 <= 4 [�? 1 分]�?
 * 在所有可能的轮调中，返回我们�?能得到的�?高分数对应的轮调下标 k 。如果有多个答案，返回满足条件的�?小的下标 k �?
